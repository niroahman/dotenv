...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME background.color=0x00000000 background.border_color=0xffeed49f background.border_width=2 label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes background.color=0x00000000 background.border_color=0xffa6da95 background.border_width=2

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME background.color=0x00000000 background.border_color=0xffeed49f background.border_width=2
